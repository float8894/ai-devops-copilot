...
if (mode === 'stdio') {
  ...
  log.info('Redis MCP server running (stdio)');
} else {
  startMcpHttp(createServer, env.MCP_REDIS_HTTP_PORT, 'mcp-redis');
  log.info(
    { port: env.MCP_REDIS_HTTP_PORT },
    'Redis MCP server running (HTTP)',
  );
}
